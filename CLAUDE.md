...
nativement par Claude via un bloc `document` base64, ou lien/ID Google Slides existant lu via
...
généré. **Pas de PowerPoint (.pptx)** — l'API Claude ne le parse pas nativement et aucun outil
de conversion n'est disponible dans cet environnement.
...
- Backend : `app/api/rapports-template/route.ts` (`@anthropic-ai/sdk`, `claude-haiku-4-5`,
