...
export default function FicheMembrePage({ params }: { params: Promise<{ id: string; membreId: string }> }) {
  ...
  // Édition scolarité (établissement + prof principal + rencontre prof)
  ...
  const [rencontreProf, setRencontreProf] = useState("")
  ...
  async function openEditScolarite() {
    ...
    setRencontreProf(sc?.Rencontre_Prof ?? "")
  ...
  async function handleSaveScolarite() {
    await addScolarite(membreId, selEtabId, selProfId, rencontreProf)
    await loadData()
    setScolOpen(false)
  }
    ...
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <SlideOver open={scolOpen} onClose={() => setScolOpen(false)} title="Modifier la scolarité" width="md">
        <form onSubmit={e => { e.preventDefault(); handleSaveScolarite() }} className="flex flex-col gap-4">
          ...
          <Field label="Rencontre prof.">
            <Select value={rencontreProf} onChange={e => setRencontreProf(e.target.value)}>
              <option value="">— Choisir —</option>
